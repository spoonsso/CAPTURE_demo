...
whos
[status,cmdout] = system('free -h','-echo');
ML_features = compute_appendage_pc_demos(modular_cluster_properties_clipped_index_8,ML_features,coeff_file,overwrite_coeff, fps);

disp("Clearing joint angle and segement length features from ML_features for memory")
ML_features = rmfield(ML_features, {'all_segments'}) %'jointangle_struct'
%% compute the wavelet transform
disp("whos11")
whos
[status,cmdout] = system('free -h','-echo');
tic
ML_features = compute_wl_transform_features_demo(ML_features,coeff_file,overwrite_coeff, fps);
toc
